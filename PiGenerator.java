...
public class PiGenerator {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("");
        System.out.println(Arrays.toString(computePiInHex(2)));
        System.out.println(Arrays.toString(computePiInHex(5)));
        System.out.println(Arrays.toString(computePiInHex(10)));


    }
    public static int[] computePiInHex(int precision) {
//        if (precision < 0) return null; //reference to nothing
        if(precision<0) return new int[0]; //reference to an array of size 0
        else {
            int[] results = new int[precision];
            if (precision >= 0) {
                for (int i =0; i < precision; i++) {
                    results[i] = piDigit(i+1);
                }
            }
            return results;
        }
